...
| # | 事项 | 优先级 | 依赖 | 工作量 | 说明 |
|---|---|---|---|---|---|
| U1 | ~~P0 延迟残留裁决~~ **已完成（2026-09-07 用户裁决关闭）** | — | — | — | 双槽改造（`-np 2` + 信号量 + 排队超时 `2a9e8ea`）后扩样实测（160 题 × 2 轮，生产模式）：端到端 P95 5.52/6.56s、P50 3.59/3.41s，320/320 零硬失败，落入 7s 预算；r2 cold-only P95 7.28s 尾部查因归为外部搜索延迟（系统本体 keyword+generation P95 合计约 4.35s，轮间漂移 <2%），不在预算口径内。详见 `reports/ask-extended-video-20260907-dualslot-summary.md`。原三选一不再需要 |
