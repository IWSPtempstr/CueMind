...
| # | 事项 | 优先级 | 依赖 | 工作量 | 说明 |
|---|---|---|---|---|---|
| U1 | P0 延迟残留裁决 | **高** | 无 | S（仅决策+文档）或 L（架构改造） | 完成 P95 7119ms > 7s 预算（±6% 噪声带）。三选一：修订决策 67 预算口径 / 继续压答案质量 / 硬件手段（投机解码、量化、升级）。**须用户裁决，不得静默实施**。2026-09-07 数据更新：双槽改造（`-np 2` + 信号量 + 排队超时）后扩样实测（160 题 × 2 轮，生产模式）端到端 P95 5.52/6.56s 已落入 7s 预算，cold 尾部由外部搜索延迟驱动（见 `reports/ask-extended-video-20260907-dualslot-summary.md`）；数据面支持收敛，裁决仍归用户 |
