    ...
    <section className="flex h-[50vh] min-h-0 w-full shrink-0 flex-col border-r border-neutral-800 lg:h-auto lg:min-w-0 lg:flex-1 lg:shrink">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-5 py-5">
        <div className="flex flex-col items-center gap-3">
          <button type="button" onClick={() => onRecordingChange(!isRecording)} aria-pressed={isRecording} aria-label={isRecording ? "停止录音" : "开始录音"} className={`flex size-24 items-center justify-center rounded-full border-2 text-4xl shadow-lg ${isRecording ? "border-red-400/60 bg-red-600" : "border-blue-500/40 bg-blue-600 hover:bg-blue-500"}`}>🎙️</button>
