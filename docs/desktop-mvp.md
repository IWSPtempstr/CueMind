...
| 2026-08-26 | WSL2 | Real llama.cpp Qwen3-4B keyword structured-output smoke | Pass: `llama.cpp 0.3.0-dev`, `Qwen_Qwen3-4B-Instruct-2507-Q4_K_M.gguf`, first 60 s window (31 segments), one keyword `"Cloud Code"`; 510 ms keyword latency on RTX 4060 Ti 8 GB (`-ngl 99`, ~4.5 GB VRAM) vs 1,793 ms CPU (`-ngl 0`) | `scripts/run-local-model-smoke.ts`; runtime report kept outside repository |
...
- Full real llama.cpp card generation and web search with two usable sources (a single CPU keyword-generation smoke is verified; search and card assembly are not).
