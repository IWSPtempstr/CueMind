...
9. **阶段 8：规模化与可观测性**：建立跨视频批处理、任务队列、资源配额、GPU/CPU 压力观测和长期回归看板；Trace、指标和报告保持本地闭环，不改变阶段 7 的数据与发布治理规则。
10. **阶段 9：受控模型演进**：基于阶段 7 的冻结集和影子运行证据评估蒸馏、EAGLE3 或新量化方案；所有替换继续遵守人工确认、版本化和可回滚要求。
11. **阶段 10：产品化交接**：交付 Docker 镜像、离线依赖/模型清单、启动与断网验证脚本、故障演练、数据治理审计和发布责任边界，完成从开发环境到可维护生产环境的交接。
