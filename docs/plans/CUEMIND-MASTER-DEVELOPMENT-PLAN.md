...
- 尚未完成多轮冻结集复测，单轮 `6778ms` 不能代表稳定 P95。
...
3. 若任一轮完成 P95 超过 7 秒，保留失败证据，不调整题集或预算，暂停并重新进行产品/推理裁决。
4. 修复仓库 fallback 单元与实际 8B 模型配置的漂移，避免回退到未评估的 4B。
5. 追加决策 67 的 `-np 1` 实测记录，并在旧路线图中回写状态；不覆盖 2026-08-29 记录。
6. 通过完整门禁后，才将 P0 标记为“已关闭（证据范围：冻结集与当前硬件）”。
...
**不在本阶段做：** EAGLE3 下载转换、SGLang/vLLM 迁移、降低回答质量、单引用改造、主动问题检测。
