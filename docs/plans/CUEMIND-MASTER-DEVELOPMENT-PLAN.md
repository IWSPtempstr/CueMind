...
**作品集目标：** 以可复现的完全离线工程作品为优先，先证明本地运行、测量、可观测、故障恢复和交接能力，再扩展长期模型能力。完全离线必须至少通过断网启动与运行测试；在此之前不得把离线能力仅作为文档声明。
...
**2026-08-30 实现记录：** 7.1–7.4 已落地。`lib/vault-governance.ts` 提供 Vault 追加版本、来源等级、冲突/高风险待审、人工审批、撤回、默认检索过滤和审计记录；概念导出成功后同步登记治理索引。MCP 新增 `search_vault`、`get_vault_entry`、`get_vault_version` 三个只读工具，治理 JSON 索引不在 MCP 进程中写入。治理单测、Vault 导出回归、TypeScript、Lint 和生产构建通过；完整 MCP 旧种子回归仍有既有 `search_cards` fixture 不匹配，未将其误报为本阶段通过。

#### SFT / DPO 数据与候选模型（决策 65）

训练只在离线环境进行，不在线改权重、Prompt 或 schema。SFT 学习窗口内容到卡片/跳过决策、关键词提示和输出格式；DPO 只使用经过人工确认的 chosen/rejected 偏好对，禁止把未核验的自动轨迹直接作为偏好标签。训练产物必须绑定数据集版本、训练配置、基础模型哈希和代码版本。

5. **7.5 训练数据治理**：从真实转录、视觉内容、卡片生成和用户裁决轨迹导出可追溯 SFT/DPO 数据；去重、脱敏、划分训练/验证/冻结集，并记录每条样本的来源和标注证据。
6. **7.6 候选模型评估**：离线训练候选模型，执行冻结集质量、schema、关键词、检索路由和拒答行为评测，再进行影子运行；人工复核后才能决定发布、保留或回滚。

训练数据以 CueMind 的真实转录、视觉内容、卡片、问答、失败轨迹和人工裁决为主，公开数据仅补充通用能力。SFT/DPO 评测至少覆盖卡片生成/跳过判断、关键词提取、垂直源与通用搜索路由、结构化 JSON、正确拒答及 ASR→卡片链路时序；不得只用主观样例或单一 JSON 合法率判定收益。

#### ASR 可靠性（决策 66）

先解决 partial/confirmed 一致性和常驻转写吞吐。`confirmed` 文本不得回退、重复或改变既有卡片绑定语义；断流、重启和服务恢复必须可验证。评测同时记录吞吐、端到端延迟、确认延迟、错误率和恢复时间，不以单一 WER/CER 替代时序验证。

7. **7.7 常驻 ASR**：实现常驻 Whisper、LocalAgreement-2、增量前缀提交和 confirmed 水位持久化；用有音频、无音频、断流、重启 fixture 验证一致性、恢复和降级路径。

#### 模型性能基线与产品交接

模型基线必须固定冻结集、硬件、模型哈希、llama.cpp 版本、Prompt/来源策略版本和数据集版本，对照 4B/8B、量化档位、KV cache 与 GPU 参数；报告质量、吞吐、显存、首 token、P95 和失败样本。评测不自动阻断发布，关键指标和风险由人工综合判断。

8. **7.8 一键评测与发布**：提供环境检查、服务启动与 health 检查、冻结集评测、ASR/模型/缓存/异常矩阵、报告归档、人工确认、版本化发布和回滚的一键流程；报告必须保留完整分母、配置快照和 evidence boundary。

**2026-08-30 实现记录：** 7.5–7.8 已落地基础交付。训练导出沿用人工裁决优先的 SFT/DPO 白名单与稳定切分；新增 ASR confirmed 单调性/重复检测和吞吐、RTF 汇总；新增固定 4B/8B、Q4/Q8、KV/GPU 基线 manifest；新增 `scripts/run-offline-eval.sh`（本地依赖检查、类型/Lint、治理/ASR/Vault 回归、基线报告）和 Docker 生产镜像。完整真实模型矩阵与影子运行仍需在目标硬件上执行并由人工确认，不自动发布。

交接优先以 Docker 镜像交付，镜像内固定运行时与依赖，外部挂载模型、配置、数据和报告目录；宿主机直接运行列为后续兼容项。离线观测数据（Trace、指标、日志、评测报告）默认全部本地保存，不上传远程平台，支持脱敏导出。

#### 正式后续阶段

9. **阶段 8：实时稳定性与本地可观测性**

以本地视频重放模拟实时会议，不建设跨机器任务队列、Redis/RabbitMQ 或多租户资源配额平台。实现可暂停/继续/重复/断点重放，观测 Whisper、LLM、GPU/CPU、显存、队列等待、端到端延迟和错误恢复；Trace、指标和报告全部本地保存。

**验收：** 固定视频集可重复回放；长时间运行无未解释的内存增长；断流、超时和资源压力均有降级与恢复证据；同一输入和配置可生成可比较的本地回归报告。

**2026-08-30 实现记录：** 新增 `lib/realtime-replay.ts`，提供暂停/继续、取消、重启、断点定位和逐步推进的本地重放控制器，以及 JSONL Trace 写入和运行汇总；`scripts/run-offline-eval.sh` 已纳入重放契约测试。跨视频长时间压力和真实 GPU/CPU 观测仍需在目标硬件上执行。

10. **阶段 9：受控模型评估（必做）**

固定阶段 7 冻结集、硬件、模型哈希、llama.cpp 版本、Prompt/来源策略版本和数据集版本，对比 4B/8B、量化、KV cache、GPU 参数及候选蒸馏/EAGLE3 方案。记录质量、吞吐、显存、首 token、P95、失败率和 ASR→卡片链路影响，执行影子运行。

**验收：** 每个候选都有完整配置快照和对照报告；发布决定由人工作出；替换必须版本化、可回滚，不允许线上自动换模、自动改 Prompt 或自动改权重。

11. **阶段 10：本地产品化交接**

交付可复现的 Docker 部署包：镜像、离线依赖与模型清单、配置模板、外部挂载目录约定、启动/停止/升级/回滚脚本、断网验证脚本、故障演练和运行手册。多用户权限、集中式日志、自动更新和商业安装器不属于本阶段。
