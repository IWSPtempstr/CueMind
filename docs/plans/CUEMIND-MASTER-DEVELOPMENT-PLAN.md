...
以 Vault 为唯一写入源，MCP 只读消费；不重新引入 Milvus 或未经决策批准的 embedding 知识库。

- 来源可信等级固定为 `primary > official > academic > community > unknown`。系统按来源类型初始标记，人工可调整，调整必须写入审计记录。
- 不自动裁决来源冲突：保留冲突关联并标记 `needs_review`，由人工确认后决定是否形成当前版本。
- 版本采用追加策略：每次变化生成新版本，旧版本保留，可回溯和比较；不覆盖历史。
- 条目最小元数据统一包含 `entryId`、标题、正文、来源 URL、来源等级、版本号、更新时间、状态和审计记录。
- 改变既有结论、关键词或行动建议的更新属于高风险更新，先进入 `pending_review`；人工确认前不得成为当前版本，也不得参与默认检索。
- 删除采用撤回而非物理删除：标记为 `withdrawn`，保留历史版本和审计记录，默认检索排除。
- MCP 只暴露三类读取能力：按关键词检索、按 `entryId` 获取当前版本、按版本号获取历史版本；不提供写入、覆盖或删除接口。

**分阶段实施与验收：**

1. **7.1 数据模型与审计**：实现 Vault 条目、追加版本、来源等级、冲突关联、`pending_review`/`needs_review`/`withdrawn` 状态和审计记录；用 fixture 验证版本回溯、撤回默认排除和人工确认流转。
2. **7.2 写入与人工裁决**：实现唯一写入路径及高风险更新审批；验证冲突不自动覆盖、审批前不可检索、审批后仅新版本成为当前版本。
3. **7.3 MCP 只读接口**：实现关键词、当前版本、历史版本三个读取接口；验证 MCP 无写权限且返回来源、版本和状态元数据。
4. **7.4 集成与门禁**：将卡片/检索结果写入治理链路，补齐重启、重复写入、并发版本和历史数据兼容测试；冻结一组治理 fixture，要求审计完整率、冲突保留率和默认检索过滤准确率 100%。
