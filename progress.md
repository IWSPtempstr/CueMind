...
## 2026-08-28 垂直源升级与落地总规划（V + P0–P5）验收结论

**执行范围**：四垂直源检索升级（V）+ 兼容性风险修复（P0）+ 会话主题摘要（P1）+ 追问 SQLite 持久化（P2）+ 桌面双轨输入源（P3）+ 转写质量提升（P4，保留 small 模型不下载 turbo Q5）+ 全量验收（P5）。

**提交序列**：c026b82(V) → 2aea3f4(P0) → e3eb354(P1) → 70902d9(P2) → 5114d07(P3) → 7538ab8/286a2f6/a3c6e0a(P4) → 6f69f39/46d6755(修复+清理)。

**性能指标实测**：
- ASR RTF：基线 0.271 → 全套参数（VAD+prompt 模板+解码调优）0.316（高密度语音片段，无静音可省；质量收益为主导）
- 转写质量：繁体无标点 → 简体带标点（150s 片段 61 处标点 vs 基线 4 处），术语 Harness 正确 5 次，字幕类幻觉 0
- 卡片链路：端到端 P50 3.3s（决策 25 目标线 P50≤5s 达标）；垂直源命中时 search 顶满 2.5s 预算、零 Tavily 消耗、trace verticalHit=true
- chat 空闲看门狗：drip 75s 长流不断（旧 60s 硬超时已消除）；30s 空闲切断
- 取消抢占：断开 2s 内 whisper 子进程归零、临时目录清理

**功能验收**：12 个回归脚本全绿；浏览器冒烟——会话下拉 [YYYY-MM-DD]-主题、追问跨刷新持久（SQLite WAL，standalone 打包自动降级 JSONL）、卡片 keyPoints 要点列表+证据折叠+来源徽标、设置面板（术语库/VAD/无 Groq）、上传流式三窗渐进、段落分组（窗标记触发）全部 PASS。

**遗留**：Windows 双轨未实机验收（WSL 限制，代码+单测已交付）；standalone 打包需手动纳入 better-sqlite3 否则 JSONL 运行（行为正确）；`transcriptionLanguage` 设置项已无调用方（无害残留）。

**版本标记**：v0.2.0 = V+P0–P2（垂直源、风险修复、会话增强、追问持久化）；v0.3.0 = P3–P5（桌面输入源、转写质量、卡片格式化）。
